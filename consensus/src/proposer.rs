...
impl Proposer {
    ...
    //     }
    // }
    async fn make_block(&mut self, round: Round, _qc: QC, _tc: Option<TC>) {
        // 使用空的 QC 和 TC 值
        let empty_qc = QC::genesis(); // 或 QC::default() 依据实际代码中可用的方法
        let empty_tc = None;          // 使用 None 作为空的 TC
        
        // Generate a new block.
        let block = Block::new(
            empty_qc,             // 使用空 QC 替代传入的 qc
            empty_tc,             // 使用空 TC 替代传入的 tc
            self.name,
            round,
            /* payload */ self.buffer.drain().collect(),
            self.signature_service.clone(),
